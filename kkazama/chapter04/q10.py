...
def calc_axis(spec, fs, S):
    F = spec.shape[0]
    T = spec.shape[1]
    L = (F - 1) * 2

    F_axis = np.arange(0, F + 1) / F * (fs / 2)
    T_axis = (np.arange(0, T + 1) * S - (L - S)) / fs
...
X = stft(x, L, S, w)
F, T = calc_axis(X, fs, S)

# プロット
fig, ax = plt.subplots(nrows=2, ncols=1, figsize=(6, 5))

p0 = ax[0].pcolormesh(T, F, np.abs(X))
ax[0].set_title("Amplitude spectrum")

p1 = ax[1].pcolormesh(T, F, np.angle(X))
